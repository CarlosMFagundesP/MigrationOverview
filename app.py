...
app.layout = html.Div([
    ...
    html.Div([
        html.H1(
            ...
            style={'width': '100%', 'display': 'inline-block', 'height': '50px', 'font-family':'sans-serif','color':'#155724', 'text-align': 'center','vertical-align': 'top', 'padding': '0px'},
        ),

        html.H2(
            "An Overview",
            style={'width': '100%', 'display': 'inline-block', 'position': 'relative','height': '5px', 'color' : '#155724', 'font-family':'sans-serif', 'text-align': 'center','vertical-align': 'top', 'padding': '0px'}
        ),
    ], className='Title'),
    ...
    html.Div([
        html.Div([dcc.Markdown('### MIGRATION AROUND THE WORLD'
                  ...
                  ], style={'width': '40%', 'border-radius': '5px', 'background-color': '#f9f9f9', 'margin': '10px',
                            'padding': '15px',
        ...
        html.Div([
            ...
            html.Div([dcc.Graph(id='choropleth_graph')
                      ], style={'width': '100%','font-family':'sans-serif', 'color': '#111', 'backgroundColor': '#fff'}, className = 'choro-map'),
        ...
        html.Div([
            ...
            html.Div([
                ...
                dcc.Slider(
                    ...
                    step=1,
                    included=False
                )], style={'width': '100%', 'text-align': 'justify', 'vertical-align': 'middle', 'margin-left': '10%',
                           'margin-right': '10%'}),
        ], style={'width': '40%'}),
        ...
        html.Div([
            html.Div([
                html.Div([html.Div([html.H4("The selected country and year are:")],
                                   style={"font-size": 15, 'horizontal-align': 'middle'}),
                          dcc.Loading(html.Div([html.H4("...")], id="box-country",
                                               style={"font-size": 20, "font-weight": "bold",
                                                      'horizontal-align': 'middle'}))],
                         className='h-auto shadow mb-3 p-4 bg-white rounded'),
            ], style={'margin-right': '5%', 'width': '100', 'text-align': 'center', 'background-color': '#f9f9f9',
            ...
            html.Div([
                html.Div([html.Div([html.H4("")], style={"font-size": 15, 'horizontal-align': 'middle'}),
                          dcc.Loading(html.Div([html.H4("")], id="box-net",
                                               style={'width': '100%', "font-size": 15, "font-weight": "bold"}))],
                         className='h-auto shadow mb-3 p-4 bg-white rounded'),
            ], style={'margin-right': '5%', 'width': '100%', 'text-align': 'center', 'background-color': '#f9f9f9',
                      'vertical-align': 'center', 'horizontal-align': 'middle'}),
                  ...
                  'margin-left': '10%', 'backgroundColor': '#f9f9f9'}),
    ], style={'display': 'flex', 'margin-left': '2%', 'margin-right': '2%', 'border-radius': '5px',
              'background-color': '#f9f9f9', 'margin': '10px', 'padding': '15px',
              'position': 'relative', 'box-shadow': '2px 2px 2px lightgrey'}),
    ...
    html.Div([
        ...
        className='pretty',
        style={'display': 'flex', 'backgroundColor': '#fff', 'font-family': 'sans-serif'}
    ),
        ...
        html.Footer([
            ...
            html.Label([" | Data available at: ",
                       ...
                       style ={'width': '100%', 'display': 'inline-block', 'color' : '#111', 'font-family':'sans-serif',
                               ...
                               'padding': '5px'}
       )])],className = 'col-3 shadow p-4 mr-4 mb-5 bg-white rounded', style = {'backgroundColor':'#f2f2f2','padding':15, 'margin-bottom':0})
...
def update_graph(countries, year):
    ...
    layout_hbar = dict(title=dict(text="Migration inflow - Top-10 countries<br>" + str(countries) + ", " + str(year),
                                  x=.5, font={"size": 14}),
                       xaxis=dict(title=dict(text="Number of migrants", font={"size": 12}),
                                  gridcolor="LightGrey",
                                  showline=True,
                                  range=[0, max_in_out],
    ...
    layout_hbar1 = dict(title=dict(text="Migration outflow - Top-10 countries<br>"
                                        + str(countries) + ", " + str(year), x=.5, font={"size": 14}),
                        xaxis=dict(title=dict(text="Number of migrants", font={"size": 12}),
                                   gridcolor="LightGrey",
                                   showline=True,
                                   linecolor="rgb(89, 89, 89)",
...
def update_graph(countries, year):
    ...
    layout_bar = dict(title=dict(text="Deaths due to <br>Conflicts and Terrorism",
                      ...
                      xaxis=dict(showline=True,
                                 ...
                                 tickangle=-90,
                                 tickfont=dict(family="Arial", size=10)),
                      yaxis=dict(title=dict(text="Number of deaths per 100000 inhab", font={"size": 12}),
                                 gridcolor="LightGrey",
                                 range=[0, 765],
                                 tickfont=dict(family="Arial", size=10)
                                 ),
    ...
    layout_bar2 = dict(title=dict(text="GDP per capita",
                       ...
                       xaxis=dict(showline=True,
                                  ...
                                  tickangle=-90,
                                  tickfont=dict(family="Arial", size=10)),
                       yaxis=dict(title=dict(text="US Dollars", font={"size": 12}),
                                  gridcolor="LightGrey",
                                  range=[0, 130000],
                                  tickfont=dict(family="Arial", size=10)
                                  ),
    ...
    layout_bar3 = dict(title=dict(text="Political stability",
                       ...
                       xaxis=dict(showline=True,
                                  ...
                                  tickangle=-90,
                                  tickfont=dict(family="Arial", size=10)),
                       yaxis=dict(title=dict(text="Stability index (-2.5 weak; 2.5 strong)", font={"size": 12}),
                                  gridcolor="LightGrey",
                                  range=[-2.5, 2.5],
                                  tickfont=dict(family="Arial", size=10),
                                  ),
    ...
    layout_line = dict(title=dict(text='Inflow vs Outlow: ' + str(countries) +
                                  ...
                                  x=.5,
                                  font={"size": 14}),
                       xaxis=dict(title=dict(text='Year',font={"size": 12}),
                                  gridcolor="LightGrey",
                                  range=[year_aux, year],
                                  showline=True,
                                  linewidth=1.1,
                                  ...
                                  tickfont=dict(family="Arial", size=10),
                                  tickmode='linear'),
                       yaxis=dict(title=dict(text="Number of migrants",font={"size": 12}),
                                  gridcolor="LightGrey",
                                  range=[0, max_in_out],
                                  tickfont=dict(family="Arial", size=10)),
                       legend=dict(orientation='h',
